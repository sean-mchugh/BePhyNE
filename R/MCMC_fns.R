...
Write_to_File = function (filename,iteration, current_vals, tree, firstline = F)
{
  ...
  if (firstline == T) {
    ...
    header= gsub(pattern= "trait1", replacement="opt_sp", x=header)
    header= gsub(pattern= "trait2", replacement="brdth_sp", x=header)
    header= gsub(pattern= "trait3", replacement="tol_sp", x=header)
    
    for(sp in 1:length(tree$tip.label)){
      
      header= gsub(pattern= paste0("_sp",sp,"$"), replacement=paste0("_",tree$tip.label[[sp]]), x=header)
    ...
    #header <- c("Iteration", names(unlist(current_vals[[1]][[2]])))
...
#R_corr_start = startPars$R$R_cor
...
metro_haste_full_MV = function (R_corr_start, R_sd_start, A_start, Prior, tree, tibble_data,
                                ...
                                plot_file, True_pars = NULL, append2existingfile=F)
{
  {
     ...
     prior.heights.lik = lapply(1:length(current_vals[[1]][[1]]),
                               function(pred) {
                                 lapply(1:length(pa_data), function(sp) {
                                   if (is.null(Prior[[pred]]$heights.prior) ==
                                       F) {
                                     Prior[[pred]]$heights.prior[[sp]](as.numeric(dat.cur[[pred]][sp,
                                                                                          3]))
                                   }
                                   else {
                                     0
  ...
  if (write_file == T) {
    ...
    if ( file.exists(paste0(filename,".pars.log")) ) {
      if(append2existingfile==T){
        

        logdf = read_BePhyNE_log(file_name = paste0(filename,".pars.log"))
        
        next_iteration = logdf[nrow(logdf),1] + 1
        logdf = NULL
        
        Write_to_File(filename = filename, iteration =  next_iteration, tree = tree,
                      current_vals = current_vals, firstline=F)
        
        next_iteration = next_iteration +1
        iterations = next_iteration +iterations
        ...
        while(file.exists(paste0(new_filename,".pars.log"))){
          int=int+1
          new_filename = paste0(filename ,"_" ,int)
          
        }

        filename = new_filename
        Write_to_File(filename = filename, iteration = 1,tree = tree,
                      current_vals = current_vals, firstline=T)
        

        cat(paste0("File already exists and option to append to existing is off writing file to new available file name:", filename, ".pars.log \n"))        
      }
  ...
  for (i in next_iteration:iterations) {
    ...
    if (i > burnin) {
      if (write_file == T) {
        if (trim == TRUE) {
          if (i%%trim_freq == 0) {
            Write_to_File(filename = filename, i,current_vals = current_vals, tree = tree, firstline = F)
          }
        }
        else {
          Write_to_File(filename = filename, i, current_vals = current_vals,tree = tree, firstline = F)
...
BePhyNE_MCMC= function (tree
  ...
  results<- metro_haste_full_MV(        R_corr_start = startPars$R$R_cor
                                      ...
                                      , True_pars = NA
                                      , k=k
                                      ,append2existingfile= append2existingfile
  )

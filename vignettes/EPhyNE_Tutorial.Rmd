...
To run EPhyNE you need a dataframe loaded in from a file format such as a ".csv" (can be loaded in from an excel spreadsheet saved as a.csv file usually) with a specific column order start at leftmost: 
-   latitude in degrees
...
lets install the package and load in load in the example file which contains Presence absence data for 98 species of Plethodontid salamanders from Eastern North America (ENA_Pleth in a lot of the code) we have two macroclimate predictors from worldclim: average annual precipitation (mm) and average annual temperature (degrees Celsisus)
...
Since tolerance are not part of the mvBM in the analysis and only estimated from the glm we will put priors on each tip. with the mean being a max likelihood estimate for tolerance Note what we are doing here is a little janky/circular as we are using max likelihood GLM(NOTE) estimates as the means, this is just a weak prior to place it somewhere between .05 and .99. You could use a flat prior but it will result in EPhyNE taking much longer to mix.  
...
Now we have all the pieces needed to construct our Prior functions, these are what will be passed through EPhyNE's MCMC to calculate the prior likelihood our parameters 

```{r}

Prior_scale <-lapply(1:pred, function(x) makePrior_ENE(r = 2, p=1, #p is the number of regimes, currently EPhyNE is only capable of modeling one, but stay tuned!
                                                       den.mu = "norm",  
...
# Simulate mvBM and use GLM estimates as starting individual species responses

#startPars_scaled$sim_dat$sim_dat_bt<- GLM_only_ml$start_pars_bt
#
#findBadStart(res=startPars_scaled$sim_dat$sim_dat_bt, pa_data=data_final, plot=F)
#  
#
#start_good<-findStart(res=startPars_scaled$sim_dat$sim_dat_bt, pa_data=data_final, tree, plot=F)
...
mcmc_lt<-EPhyNE_out2coda_mcmc(results, tree)
...
lets plot some MCMC chains, we can either do that directly for specific parameters using traceplot and dense plot, or plotting all of them using PlotEPhyNEchains()
fn, which will return a set of files with the chains and/or densities for each parameter either to your work directory or a specified directory, writing these plots to files is usually the best move as there are a lot of parameters (species responses AND mvBM poars....thats a lot)

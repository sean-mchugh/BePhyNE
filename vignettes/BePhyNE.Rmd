...
Now that we have our dataset we can start the exciting task of setting our priors, we will inform our parameters with informative/weakly informative priors (entirely uninformative uniform priors can lead to some weird behavior especially for niche breadth and mvBM pars). "make_all_priors is preloaded with presetted values (explictly written out in the arguments). 


Since tolerance are only estimated from the glm, each species has a separate prior for tolerance. Some option for setting these include using the max likelihood estimate for tolerance as the mean of the normal distribution, or a fixed prior across all species. For simplicity we apply a weak Normal prior with a mean of 0.95 on tolerance. You could use a flat prior but it will result in BePhyNE taking much longer to mix.  


We do not believe this actually reflects prevalence, but instead error in sampling of the occurrences (broad scale gbif data) and generated absence data (using target group absences filtered to an equal number within the species IUCN expert range map and double the number of presences outside the expert range), see the pre print for details
...
Prior_scale = make_all_priors(N = Npred, 
                              ...
                              heights_mean_by_sp    = sample(.95, size = Ntips, replace = TRUE),
...
To get start values we simulate parameters of our model, including individual species response curves and mvBM pars, however because we only can set what the mvBM pars are and the tip responses are subsequently simulated, we don't have a lot of control over what the tip values will be and some potential starting values could be so far off they cause the likelihood function to return nonsensical values, breaking the MCMC. So it may take a lot of trie (each iteration number printed out in console) to reach a set of "good" starting parameters. Sometimes it can be difficult to draw a set of good starting parameters from a single model draw, if that is the case, starting pars can be drawn for each remaining species with "bad" response curves using a process-of-elimination. This obviously is less ideal so we only do that if we fail at getting good starting parameters after a LOT of draws (in this case 1000), you can dial this however you like. 

```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
startPars = get_starting_values(Prior_scale = Prior_scale, tree=tree,data = data_final, reps_before_POE = 1000)
...
Next you need to set your tuning parameters, how often do we want to be making proposals on any specific parameter, we set these through trial and error on our scaled predictors, each tip response par and mvBM par has its own tuning par in this setup, each response curve proposal type (center_lide, center_mult, width_slide, etc) has its own tuning parameter. Many of these are set to what we found to be "reasonable" but one can set these however they like, set to lower values if you find that you have poorly mixing changes with many big jumps and few accepted proposals, and set to higher values if MCMC chains look autocorrelated across samples.  
...
contsimmaps = make_simmaps_BePhyNE( tree = tree
                                   , logdf = logdf
                                   , nsims      = 10
...
```{r fig.height=8, fig.width=14, message=FALSE, out.width="100%"}
